...
* **Projeto de Validação**: Projeto Exemplo (Módulo de Estoque e Fábrica de Stores CRUD)
...
## 2. Instanciação Prática da Rastreabilidade no Piloto de Demonstração

A tabela a seguir mapeia os registros físicos que garantem a rastreabilidade do piloto de refatoração das stores do projeto alvo:
...
| **4. Código** | `src/estados/criarStoreCrud.ts` | `Código` | Refatoração física removendo `import ... from 'firebase/firestore'`. |
| **5. Teste** | `src/estados/criarStoreCrud.test.ts` | `Mocks` | Validação unitária das stores sem usar dependências reais do Firebase. |
...
O piloto do EOS no projeto alvo será considerado bem-sucedido quando:
